---
title: "`r params$unit_name` Historical Fire Regimes"
author: "Randy Swaty"
date: "`r Sys.Date()`"
output: 
  html_document:
...
params:
  ...
  unit_name: "Acadia National Park"
---
...
data <- read.csv("inputs/bps_scls_nps_cmbn.csv") |>
  clean_names() |>
  #filter(region == "9") |>
...
bps_name_df <- data %>%
  filter(unit_name == params$unit_name) %>%
  group_by(bps_name) %>%
...
In these reports we do quick summaries of the BpSs and their associated historical fire regime attributes for each National Park to:

1. Identify the top Biophysical Settings and their extent
2. Calculate average annual acres burned historically
3. Explore which fire types were most prevalent (i.e., surface, mixed or replacement)
...
From a quick assessment of the BpSs and their historical Mean Fire Return Intervals for **`r unit_name`** we found:
...
* For the 10 most common BpSs, roughly **`r scales::comma(total_acres_burned)`** acres burned annually or **`r percent_of_roi_burned`%** of **`r unit_name`**.
